...
| V6 | causal pre-window series memory adapter | train-only audit CLI/test/runner 구현 및 5090 `7/7` sync; tmux 미실행, adapter 미구현 | `SELECTED_HYPOTHESIS` | Taxi design/audit only; no quality claim |
...
- V6는 기존 `series_lmm` switch가 아니라 static LMM을 유지하는 zero-init causal
  pre-window adapter로 선택했다. Train-only audit 구현과 5090 source checksum
  동기화는 완료했지만 tmux는 아직 실행하지 않았고 Taxi 결과도 읽지 않았다.
