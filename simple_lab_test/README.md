...
초기에는 별도 hyperparameter-search 스크립트로 아래 조합을 자동 탐색했습니다.
현재는 실행 진입점을 `search/tpp_experiment.py long-epoch`로 통합하고,
Titan 후보와 scale base 관련 공통 로직은 `search/common/`으로 이동했습니다.
...
탐색 결과를 바탕으로 dataset별 최적 Titan 조합을 사용해 30 epoch 기준
A/B 테스트를 수행했습니다. 현재 재실행은
`search/tpp_experiment.py long-epoch --models rmtpp,titantpp`를 사용합니다.
