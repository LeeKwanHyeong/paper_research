...
TMUX_SESSION="${TMUX_SESSION:-titantpp_v7_taxi_time_source_audit_0719}"
EXECUTION_SERVER="${EXECUTION_SERVER:-5080}"
OUTPUT_DIR="${PROJECT_ROOT}/search_artifacts/model_enhancement_titantpp_v7_taxi_time_source_audit_0719"
...
if [[ "${EXECUTION_SERVER}" != "5080" ]]; then
  echo "This audit is locked to execution server 5080; received ${EXECUTION_SERVER}." >&2
  exit 2
fi
...
"${PYTHON_BIN}" simple_lab_test/search/analyze_taxi_time_source_isolation_audit.py \
  --dataset "${DATASET}" \
