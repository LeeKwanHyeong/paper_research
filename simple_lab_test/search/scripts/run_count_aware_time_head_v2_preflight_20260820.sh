...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/search_artifacts/count_aware_time_head_v2_preflight_20260820}"
...
SOURCE_FILES=(
  ...
  "${PROJECT_ROOT}/simple_lab_test/search/scripts/run_count_aware_time_head_v2_preflight_20260820.sh"
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/mplconfig_time_head_v2_preflight}"
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/xdg_time_head_v2_preflight}"
export TORCHINDUCTOR_CACHE_DIR="${TORCHINDUCTOR_CACHE_DIR:-/tmp/torchinductor_time_head_v2_preflight}"
...
"${PYTHON_BIN}" "${PROJECT_ROOT}/paper/scripts/run_count_aware_tpp_backbone_control.py" \
  ...
  --backbones titantpp_persistent_only \
  --seeds 42 \
  --max-train-batches 2 \
  --max-val-batches 2 \
...
"${PYTHON_BIN}" "${PROJECT_ROOT}/paper/scripts/run_count_aware_time_head_stability.py" \
  --data "${INTER_DATA}" \
  --split-manifest "${INTER_MANIFEST}" \
  --output-dir "${OUTPUT_ROOT}/intermittent_train_only_short" \
  --source-revision "${SOURCE_REVISION}" \
  --execution-role primary_5080_train_only_short_preflight \
  --device cuda \
  --epochs 1 \
  --batch-size 128 \
  --lr 1e-3 \
