...
case "${SHARD_ID}" in
  ...
  seed62_5080)
    ...
    ;;
  *)
    printf 'Unsupported TitanTPP-MAC shard: %s\n' "${SHARD_ID}" >&2
    exit 2
    ;;
esac
...
"${PYTHON_BIN}" "${VALIDATOR}" verify-source \
  ...
  --contract "${CONTRACT}" \
  --output "${OUTPUT_ROOT}/preflight/frozen_source_validation_${SHARD_ID}.json"
"${PYTHON_BIN}" "${VALIDATOR}" verify-split \
  --contract "${CONTRACT}" \
  --split-contract "${SPLIT_CONTRACT}" \
  --output "${OUTPUT_ROOT}/preflight/split_contract_validation_${SHARD_ID}.json"

if [[ "${VERIFY_ONLY}" == "1" ]]; then
  printf 'TitanTPP-MAC %s source and split verification passed.\n' "${SHARD_ID}"
  FINALIZED=1
  exit 0
fi
...
} > "${OUTPUT_ROOT}/source_manifest_${SHARD_ID}.txt"

exec > >(tee -a "${OUTPUT_ROOT}/logs/launcher_${SHARD_ID}.log") 2>&1
export PYTHONHASHSEED=42 CUBLAS_WORKSPACE_CONFIG=:4096:8
export CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 MPLBACKEND=Agg
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/mplconfig_titantpp_mac_${SHARD_ID}}"
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/xdg_titantpp_mac_${SHARD_ID}}"
cd "${PROJECT_ROOT}"
...
run_one() {
  ...
  if "${PYTHON_BIN}" "${VALIDATOR}" validate-run \
    ...
    write_status running \
      "Reused validated TitanTPP-MAC shard run ${ordinal}/${TOTAL_RUNS}."
    return
  fi

  gpu_preflight
  write_status running \
    "Starting isolated TitanTPP-MAC shard run ${ordinal}/${TOTAL_RUNS}."
  "${PYTHON_BIN}" "${POLICY_RUNNER}" \
    ...
    --execution-role "primary_${EXECUTION_SERVER}_titantpp_mac_${dataset}_seed${seed}_e300" \
  ...
  COMPLETED_RUNS=$((COMPLETED_RUNS + 1))
  write_status running \
    "Validated TitanTPP-MAC shard run ${ordinal}/${TOTAL_RUNS}."
}
...
CURRENT_SEED=""
"${PYTHON_BIN}" "${VALIDATOR}" finalize-shard \
  --output-root "${OUTPUT_ROOT}" \
  --contract "${CONTRACT}" \
  --split-contract "${SPLIT_CONTRACT}" \
  --shard-id "${SHARD_ID}"
write_status complete \
  "All ${TOTAL_RUNS} TitanTPP-MAC ${SHARD_ID} runs passed validation."
FINALIZED=1
