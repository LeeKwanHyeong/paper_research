...
SOURCE_FILES=(
  ...
  "${PROJECT_ROOT}/simple_lab_test/search/scripts/run_count_aware_scaled_time_persistent_dual_cuda_smoke_20260819.sh"
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/mplconfig_scaled_time_memory_smoke}"
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/xdg_scaled_time_memory_smoke}"
export TORCHINDUCTOR_CACHE_DIR="${TORCHINDUCTOR_CACHE_DIR:-/tmp/torchinductor_scaled_time_memory_smoke}"
...
"${PYTHON_BIN}" -m pytest \
  ...
  simple_lab_test/search/tests/test_count_aware_titan_persistent_dual_memory.py \
  -q
