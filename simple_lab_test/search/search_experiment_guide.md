...
memory를 별도 masked residual adapter로 추가하는 설계였습니다. Train-only
audit의 final primary와 bootstrap gate가 실패해 adapter 구현 전에 종료했습니다.
`M=64/topk=4`는 구현 상수로 동결하지 않으며 Taxi incumbent는 V3b입니다.

V6 train-only audit은 아래 runner로 수행했습니다. 이 runner는 Taxi train parquet,
5090, source revision, `lookback=168`, `max_seq_len=256`을 고정했고
validation/test를 읽지 않았습니다. Runner는 재현용으로 남기되 V6 품질 실험을
추가로 열지 않습니다.
