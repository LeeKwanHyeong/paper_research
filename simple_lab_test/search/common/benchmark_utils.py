"""
Shared benchmark helpers for the unified TPP experiment CLI.

The old RMTPP-vs-TitanTPP A/B script has been removed from the search root.
Dataset profile defaults, marked-cache preparation, and paper-table helpers are
kept here so `tpp_experiment.py long-epoch` and the legacy-compatible modes can
reuse them without exposing another executable script.
"""
...
from simple_lab_test.search.common.experiment_utils import (
    ...
    ensure_dir,
    prepare_fixed_split_marked_dataset,
...
def make_search_cfg(ab_cfg: ABConfig, dataset_kind: str | None = None) -> SearchConfig:
    ...
    search_cfg = SearchConfig(
        ...
        value_head_activation=getattr(ab_cfg, "value_head_activation", "sigmoid"),
...
def make_dataset_specs(ab_cfg: ABConfig, selected_names: set[str]) -> list[DatasetSpec]:
    ...
    for spec in default_dataset_specs():
        ...
        if spec.name == "intermittent":
            overrides = {
                "max_series": ab_cfg.intermittent_max_series,
                "split_with_path": getattr(ab_cfg, "intermittent_split_with_path", None) or spec.split_with_path,
                "split_train_path": getattr(ab_cfg, "intermittent_split_train_path", None) or spec.split_train_path,
                "split_validation_path": (
                    getattr(ab_cfg, "intermittent_split_validation_path", None)
                    or spec.split_validation_path
                ),
                "split_test_path": getattr(ab_cfg, "intermittent_split_test_path", None) or spec.split_test_path,
                "split_manifest_path": (
                    getattr(ab_cfg, "intermittent_split_manifest_path", None)
                    or spec.split_manifest_path
                ),
            }
            spec = DatasetSpec(**{**asdict(spec), **overrides})
        elif spec.name == "yellow_trip_hourly":
            # `yellow_trip_hourly` is a preprocessed event table generated by
            ...
            spec = DatasetSpec(**{**asdict(spec), "max_series": ab_cfg.insta_max_series})
        specs.append(spec)
    return specs
    ...
    for spec in dataset_specs:
        for dataset_name, scale_base in sorted(needed_pairs):
            if spec.name != dataset_name:
                continue
            if getattr(ab_cfg, "split_mode", "internal") == "fixed":
                marked_df, marked_meta, cache_root = prepare_fixed_split_marked_dataset(
                    spec=spec,
...
def save_learning_curve_plots(history_df: pl.DataFrame, plots_dir: Path) -> None:
    ...
    ensure_dir(plots_dir)
    # Split validation NLL into mark/time components so the paper plots show
    # which head is responsible for each model's likelihood behavior.
    curve_metrics = [
