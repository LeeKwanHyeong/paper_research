...
This study evaluates TitanTPP on three demand-event datasets with different sequence and quantity characteristics. Intermittent contains many part-level demand sequences, Taxi contains long grid-cell event histories with heavy-tailed pickup counts, and Instacart contains user-level basket sequences. All models are evaluated under fixed chronological splits, three random seeds, and the same validation checkpoint rule. The empirical analysis focuses on whether the proposed quantity-aware TPP formulation improves validation negative log-likelihood and quantity reconstruction while preserving reasonable event-time prediction.
...
RMTPP-matched and THP-matched are adapted baselines. They retain the encoder family of each original model, but they share the paper's residual quantity input, hybrid objective, output interface, fixed split, and checkpoint rule. This naming avoids presenting the runs as exact reproductions of the original RMTPP or THP papers. All models use seeds 42, 52, and 62, AdamW with learning rate 0.001, batch size 128, strict reproducibility mode, and minimum validation total NLL as the checkpoint rule. The held-out test split is evaluated once after validation-based model selection.
