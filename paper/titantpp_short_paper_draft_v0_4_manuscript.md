...
Temporal point processes offer a natural starting point because they model event arrivals conditional on prior event trajectories. Deep neural TPP models extend classical intensity-based formulations by learning history representations from observed event sequences, which allows the model to encode irregular temporal dependencies rather than relying only on hand-designed history effects. RMTPP is an early and influential example of this direction: it embeds previous events into a recurrent hidden state and predicts the next event time and mark from that state [1]. In demand-event sequences, however, this interface exposes three difficulties. First, a fixed-dimensional recurrent state can compress long histories in a way that weakens access to older demand patterns and recent local shifts. Second, categorical marks are well suited to event types, but they do not directly reconstruct continuous and heavy-tailed demand quantities. Third, rare large-demand events can dominate a direct regression objective, and the resulting gradient scale can interfere with time and mark likelihood learning when all heads update the same representation.
