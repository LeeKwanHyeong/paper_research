#!/usr/bin/env python3
"""5090-only full-context qualification followed by four fresh seed52 runs."""
...
def verify_inputs(project, contract):
    ...
    if (prior.get("status") != "complete" or prior.get("seed") != 52
...
def check_gpu(gpu_csv, compute_csv, gdm, process_names, minimum=12000):
    ...
    if len(rows) != 1:
        raise ValueError("Expected exactly one RTX 5090")
    name, free = (field.strip() for field in rows[0].split(","))
    if "RTX 5090" not in name or int(free) < minimum:
...
def training_command(project, output, contract, dataset, phase, python):
    ...
    run = output / phase / dataset / "seed_52"
    cmd = [python, DIAGNOSTIC, "--snapshot-dir", str(run.parent / "diagnostic"),
           ...
           "--source-revision", REVISION, "--execution-role", f"stable_5090_{phase}_seed52",
           ...
           "--backbones", "titantpp_titans_mac", "--seeds", "52",
...
def execute(args):
    ...
    ensure_fresh(output)
    with (project / ".titantpp_mac_stable_5090.lock").open("a") as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        state = {"state": "starting", "execution_server": "5090", "seed": 52,
            ...
            for phase, datasets in (("context_e1", contract["context_preflight_order"]),
                ...
                for dataset in datasets:
                    ...
                    run([args.python, VALIDATOR, "--project-root", str(project),
                         ...
                         "--expected-revision", REVISION, "--seed", "52", "--dataset", dataset,
                         "--epochs", str(epochs), "--min-epochs", str(minimum),
