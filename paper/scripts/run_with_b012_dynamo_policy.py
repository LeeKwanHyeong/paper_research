#!/usr/bin/env python3
"""Run a frozen B0/B1/B2 entrypoint with a bounded Dynamo shape policy."""
...
def configure_dynamo(*, recompile_limit: int, accumulated_limit: int) -> dict[str, int]:
    if recompile_limit < 1:
        raise ValueError("recompile_limit must be positive")
    if accumulated_limit < recompile_limit:
        raise ValueError("accumulated_limit must be at least recompile_limit")

    import torch._dynamo.config as dynamo_config

    dynamo_config.recompile_limit = int(recompile_limit)
    dynamo_config.accumulated_recompile_limit = int(accumulated_limit)
    return {
...
def main() -> None:
    ...
    policy = configure_dynamo(
        recompile_limit=args.recompile_limit,
        accumulated_limit=args.accumulated_recompile_limit,
    )
    print(
        "[b012-dynamo-policy] "
        f"recompile_limit={policy['recompile_limit']} "
        "accumulated_recompile_limit="
        f"{policy['accumulated_recompile_limit']}",
        flush=True,
    )
