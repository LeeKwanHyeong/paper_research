...
We compare TitanTPP with two quantity-aware neural TPP baselines, denoted RMTPP-Q and THP-Q, where Q indicates the shared quantity-aware adaptation. The original RMTPP and THP formulations predict the next event time and a categorical mark. This interface is appropriate when marks denote event types, but it does not directly reconstruct a continuous demand quantity. Replacing the categorical mark with raw quantity regression is also problematic for intermittent demand because rare large orders can dominate the regression loss. To isolate the effect of history representation, RMTPP-Q and THP-Q retain their original encoder families while sharing the same quantity interface used by TitanTPP: a transformed magnitude mark, a residual decoder, a hybrid training objective, fixed chronological splits, three random seeds, and the same validation checkpoint rule. RMTPP-Q therefore represents the recurrent-history baseline, THP-Q represents the attention-based TPP baseline, and TitanTPP tests whether a Titan-style history encoder improves event-demand modeling under the same quantity-aware formulation. All models use seeds 42, 52, and 62, AdamW with learning rate 0.001, batch size 128, strict reproducibility mode, and minimum validation total NLL as the checkpoint rule. Values are reported as mean ± standard deviation over the three seeds. The held-out test split is evaluated once after validation-based model selection.
